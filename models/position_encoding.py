...
class LearnablePositionEmbedding1D(nn.Module):
    """
    A learnable version of the position embedding, which can be optimized during training.
    """
    def __init__(self, num_pos_feats=256, temperature=10000, normalize=False, scale=None):
        super().__init__()
        self.num_pos_feats = num_pos_feats
        ...
        self.position_embeddings = nn.Parameter(torch.randn(1, num_pos_feats, 1))
    ...
    def forward(self, tensor_list: NestedTensor):
        ...
        # Use the learnable position embeddings
        pos = self.position_embeddings.repeat(x.size(0), 1, x.size(2))

...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, tgt, fp, masks, pos_embeds, query_embed=None):
        ...
        if self.two_stage:
            ...
            fp = rearrange(fp, 'b t q c -> (b t) q c')
            query_embed = query_embed + fp

            reference_points = self.reference_points(query_embed).sigmoid()
            init_reference_out = reference_points

        # decoder
        hs, inter_references, inter_samples = self.decoder(tgt, fp, reference_points, memory,
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, fp, query_pos, reference_points, src, src_spatial_shapes, level_start_index, src_padding_mask=None):

        query_pos = query_pos + fp
...
class DeformableTransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, fp, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            output, sampling_locations, attention_weights = layer(output, fp, query_pos, reference_points_input,

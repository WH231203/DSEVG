...
#####################新数据集#########################################################
def collate_fn_new(raw_batch):
    raw_batch = list(zip(*raw_batch))

    img = torch.stack(raw_batch[0])

    # print(type(raw_batch[1]))
    # print(len(raw_batch[1][0]))
    # print(raw_batch[1][1])

    masks = [torch.tensor(mask['mask']) for mask in raw_batch[1]]
    ...
    raw_batch[0] = img_data

    return tuple(raw_batch)
#####################新数据集#########################################################
...
# #################0710加：添加一个保存最好结果权重的函数（有时候最后一轮并不是最好的结果）###########
# def save_checkpoint(state, is_best, checkpoint_path, output_dir):
#     if is_main_process():
#         torch.save(state, checkpoint_path)
#         # torch.save(state, checkpoint_path, _use_new_zipfile_serialization=False)
#         if is_best:
#             shutil.copyfile(checkpoint_path, output_dir / 'checkpoint_best.pth')
# #################0710加：添加一个保存最好结果权重的函数（有时候最后一轮并不是最好的结果）###########
